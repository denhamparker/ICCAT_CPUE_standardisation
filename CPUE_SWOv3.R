...
#Choose species
  ...
  f1 = formula(y~as.factor(Year))
  
  # Basic
  f2 = formula(y~as.factor(Year)+s(Month,bs="cc",k=8))
  
  # Spatial
  f3 = formula(y~as.factor(Year)+s(Month,bs="cc",k=8)+s(Lat,Long,k=10))
  
  # Tactic 
  f4 = formula(y~as.factor(Year)+s(Month,bs="cc",k=8)+s(Lat,Long,k=10)+as.factor(FT))
  
  
  # optimise tweedie with full model (m3)
  p_i = seq(1.1,1.8,0.05)
  
  LL=NULL
  for(l in 1:length(p_i)){
    if(l==1) cat(paste0("\n","Running Tweedie Profiling","\n"))
    if(l==1) cat(paste("\n","|"))
    cat("*")
    LL[l] = logLik(bam(f4,data = final_dat,family=Tweedie(p=p_i[l])))[1]
  ...
  m0 <- gam(f0,data = final_dat,family=Tweedie(p=p_opt))
  m1 <- gam(f1,data = final_dat,family=Tweedie(p=p_opt))
  m2 <- gam(f2,data = final_dat,family=Tweedie(p=p_opt))
  m3 <- gam(f3,data = final_dat,family=Tweedie(p=p_opt))
  m4 <- gam(f4,data = final_dat,family=Tweedie(p=p_opt))
  
  # Produce model stats
  bic.note = BIC(m0,m1,m2,m3,m4)
  bic = BIC(m0,m1,m2,m3,m4)[,2]
  ...
  dDev = PercDev = 0
  for(i in 1:4){
    deviance[i+1] = deviance(get(paste0("m",i)))  
    dDev[i+1] = deviance[i+1]-deviance[i]
  }
  for(i in 1:4) PercDev[i+1] = dDev[i+1]/sum(dDev)
  
  # get p-value
  pvs = c(0,as.numeric(anova(model$gam)$pTerms.pv),as.numeric(anova(model$gam)$s.pv))[c(1,2,4,5,3)]
  
  pv.print = ifelse(pvs<0.001,"< 0.001",ifelse(pvs<0.01,"< 0.01",ifelse(pvs<0.05,"< 0.05",round(pvs,3))))
  pv.print[1] = ""
  # Make model stats table for fixed effects
  Model.stats = data.frame(Fixed.Eff=c("NULL","Year","Month","s(Lat,Long)","Fishing Cluster") ,df,AIC=aic,BIC=bic,deviance,dDev,Percent.explained=round(100*PercDev,2),p=pv.print)
  
  write.csv(Model.stats,paste0(assessment,"/Stats_",species,".csv"))
            
  # choose model with lowest BIC automatically
  best = bic.note[bic==min(bic),]
  ...
  lines(c(p_opt,p_opt),c(-10^10,max(LL)),lty=2)
  dev.off()
  ...
  #predict year while integrating over the spatial grid
  ...
  {  
    ...
    log.y[i,] = p$fit
    se = p$se.fit
    se2[i,] = se^2 # get se^2 for each grid-year combination  
   
  }
  logmu.Y = apply(log.y,2,mean)
  mu.Y = exp(logmu.Y) # average over all spatial cells
  se.Y = sqrt(apply(se2,2,mean)) # same get se = sqrt(mean se^2) 
  ...
  for(m in 1:5){
    ...
    cpue <- mu.Y  
    cpue.se <- se.Y
    lci = exp(log(cpue)-1.96*se.Y)   
    uci = exp(log(cpue)+1.96*se.Y)   
    normI = cbind(normI,cpue/mean(cpue))
    assign(paste0("Index_",species,"_",models.all[m]),data.frame(Year=pred_dat[,1],nominal=nom,cpue,cpue.se,lci,uci))
    write.csv(get(paste0("Index_",species,"_",models.all[m])),paste0(assessment,"/Index_",species,"_",Fleets[k],models.all[m],".csv"))
    } else {
    pred <- predict(get(paste0("m",m)),pred_dat,type="link",se=T) 
    cpue <- exp(pred$fit)  
    cpue.se <- (pred$se)
  ...
  plot(years,years,type="n",ylim=range(c(0.8*lcl.Y),c(1.05*ucl.Y)),xaxt="n", xlab="",ylab="Standardized CPUE")
  axis(1,labels = F)
  ...
  par(Par)
  hist(round(final_dat$y,0),freq=T,breaks=100,xlab="CPUE (kg/1000 hooks)",main="")
  hist(round(final_dat$y,0),freq=F,breaks=100,xlab="CPUE (kg/1000 hooks)",main="")
  ...
  hist(round(tweedie.dist,0),breaks=200,freq=F,col=rgb(1,0,0,0.5),add=T)
  dev.off()
  ...
  plot(1:12,mu.m,type="n",ylim=range(c(0),c(1.05*ucl.m)),xaxt="n", xlab="Month",ylab="Standardized CPUE")
  ...
  mtext("Standardized CPUE",2,outer=T,line=1,cex=0.9)
  dev.off()
